...
def set_log_sink(fn):
    """Let the GUI (or any host) receive log lines emitted from inside this module."""
    global LOG_SINK
    LOG_SINK = fn
...
async def unhandled_exception_handler(request, exc):
    ...
    return JSONResponse(status_code=500, content={"success": False, "detail": f"{type(exc).__name__}: {exc}"})
...
def discovery_loop():
    ...
    while True:
        current_ip = local_ip()
        if current_ip != bound_ip:
            try:
                beacon.bind((current_ip, 0))
                bound_ip = current_ip
                log(f"Discovery beacon bound to {current_ip}:{DISCOVERY_PORT}")
            except OSError as exc:
                log(f"DISCOVERY BIND ERROR ({current_ip}): {exc}")
        ...
        try:
            ...
            if now - last_error_logged > 30:
                log(f"DISCOVERY SEND ERROR: {exc}")
