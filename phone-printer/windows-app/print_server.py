...
APP_PORT = 8765
DISCOVERY_PORT = 8766
DB_PATH = Path(os.getenv("LOCALAPPDATA", ".")) / "PhonePrinterHub" / "jobs.db"
DB_PATH.parent.mkdir(parents=True, exist_ok=True)

app = FastAPI(title="Phone Printer Hub", version="0.2.0")
...
def print_document(path, printer_name, copies, color, duplex, orientation, paper_size):
    ...
    try:
        ...
        for copy_no in range(max(1, min(copies, 99))):
...
<label>Printer</label><select id='printer' name='printer'></select>
<label>File</label><input type='file' name='file' accept='.pdf,image/*' required>
<label>Copies</label><input type='number' name='copies' value='1' min='1' max='99'>
<label>Color</label><select name='color'><option value='true'>Color</option><option value='false'>Black & White</option></select>
<label>Duplex</label><select name='duplex'><option>Simplex</option><option>Long edge</option><option>Short edge</option></select>
<label>Orientation</label><select name='orientation'><option>Portrait</option><option>Landscape</option></select>
<label>Paper size</label><select name='paper_size'><option>A4</option><option>Letter</option><option>Legal</option><option>A5</option></select>
<input type='hidden' name='source' value='Browser'><button type='submit'>PRINT</button></form></div>
<div class='card'><h2>Recent jobs</h2><table id='jobs'><tr><th>File</th><th>Printer</th><th>Status</th><th>Time</th></tr></table></div>
<script>
async function refresh(){let p=await fetch('/printers').then(r=>r.json());document.getElementById('printer').innerHTML=p.printers.map(x=>`<option>${x}</option>`).join('');let j=await fetch('/jobs').then(r=>r.json());document.getElementById('jobs').innerHTML='<tr><th>File</th><th>Printer</th><th>Status</th><th>Time</th></tr>'+j.jobs.slice(0,20).map(x=>`<tr><td>${x.file_name}</td><td>${x.printer}</td><td class='${x.status==='failed'?'bad':'ok'}'>${x.status}${x.error?' - '+x.error:''}</td><td>${x.created_at}</td></tr>`).join('')};refresh();setInterval(refresh,3000)
</script></body></html>
"""
...
def retry(job_id: str):
    ...
    if not item:
        raise HTTPException(status_code=404, detail="Job not found")
    raise HTTPException(status_code=409, detail="Retry requires the original file; resubmit it from the client")
...
async def print_file(
    ...
    source: str = Form("Unknown device"),
):
    ...
    if duplex not in {"Simplex", "Long edge", "Short edge"}:
        raise HTTPException(status_code=400, detail="Invalid duplex option")
    if orientation not in {"Portrait", "Landscape"}:
        raise HTTPException(status_code=400, detail="Invalid orientation")
    if paper_size not in PAPER_IDS:
...
if __name__ == "__main__":
    print(f"Phone Printer Hub: http://{local_ip()}:{APP_PORT}")
