...
PDF bytes are read from private Backblaze B2 and uploaded directly to Telegram.
No Google Drive/B2/public URL is included in the Telegram message.
"""
import html
import io
import os
...
def already_sent(document_id, chat_id):
    q = ("document_operations_audit?select=id&document_id=eq." + quote(str(document_id), safe="") +
         "&operation=eq.telegram_publication_notification&details->>chat_id=eq." + quote(str(chat_id), safe="") + "&limit=1")
...
def main():
    verify_output_bot()
    rows = db_get("documents?select=id,original_filename,display_filename,subject,short_description,issuing_authority,reference_number,normalized_issue_date,received_at,published_at,category,category_key,publication_status,approved_for_publication&publication_status=eq.Published&approved_for_publication=is.true&order=received_at.desc.nullslast,published_at.desc.nullslast,normalized_issue_date.desc.nullslast&limit=50")
    sent = failed = skipped = 0
    for serial_index, doc in enumerate(rows, start=1):
        intake = db_get("telegram_intake?select=id,telegram_chat_id,metadata&document_id=eq." + quote(str(doc["id"]), safe="") + "&limit=20")
        ...
        if not object_key:
            failed += 1
            print(f"No B2 object key for document={doc['id']}")
            continue
        try:
        ...
        for chat_id in targets:
            ...
            try:
                ...
                db_insert_audit({
                    ...
                    "details": {"chat_id": chat_id, "bot_username": EXPECTED_BOT_USERNAME, "delivery": "private_b2_to_telegram_multipart", "external_link_exposed": False, "sent_at": datetime.now(timezone.utc).isoformat(), "serial_no": serial_index},
                })
                sent += 1
