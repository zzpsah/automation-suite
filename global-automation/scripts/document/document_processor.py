...
def db_insert(table,payload):
    r=requests.post(f"{SUPABASE_URL}/rest/v1/{table}",headers={**HEADERS,"Content-Type":"application/json","Prefer":"return=representation"},json=payload,timeout=30); r.raise_for_status(); data=r.json(); return data[0] if data else payload
...
def claim_intake(record_id):
    ...
    r=requests.patch(f"{SUPABASE_URL}/rest/v1/{path}",headers={**HEADERS,"Content-Type":"application/json","Prefer":"return=representation"},json={"status":"Processing"},timeout=30); r.raise_for_status(); data=r.json(); return data[0] if data else None
...
def main():
    ...
    else: rows=db_get("telegram_intake?select=*&or=(status.eq.Stored,status.eq.Processing%20Failed)&order=received_at.asc&limit=10")
