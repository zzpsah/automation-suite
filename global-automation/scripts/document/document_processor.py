...
def db_patch(table,record_id,payload):
    r=requests.patch(f"{SUPABASE_URL}/rest/v1/{table}?id=eq.{record_id}",headers={**HEADERS,"Content-Type":"application/json","Prefer":"return=minimal"},json=payload,timeout=30); r.raise_for_status()
...
def db_insert(payload):
    ...
    data=r.json(); return data[0] if data else {}
...
def first_match(text,patterns):
    for pattern in patterns:
        ...
        if m and m.group(1).strip(): return m.group(1).strip(" :-–—\t")[:500]
    return ""
...
def extract_metadata(text,filename):
    ...
    return subject[:1000],authority[:500],ref_no[:250],printed,normalize_date(printed),short[:500],detailed[:4000]
...
def process(row):
    ...
    subject,authority,ref_no,printed,normalized,short,detailed=extract_metadata(text,row.get("file_name") or "document")
    payload={
      "id":str(uuid.uuid4()),"source_app":"UMVInputBot","source_location":"Telegram","source_message_id":str(rid),
      "original_filename":row.get("file_name"),"display_filename":row.get("file_name"),"mime_type":row.get("mime_type"),"file_size":len(data),"file_checksum":storage.get("sha256"),
      "private_drive_file_id":storage.get("drive_file_id"),"private_drive_url":(f"https://drive.google.com/file/d/{storage.get('drive_file_id')}/view" if storage.get("drive_file_id") else None),"public_file_url":"",
      "reference_number":ref_no or None,"issue_date_as_printed":printed or None,"normalized_issue_date":normalized,"received_at":row.get("received_at"),"issuing_authority":authority or None,"subject":subject or None,
      "short_description":short,"detailed_summary":detailed,"category":"Other","subcategory":None,"priority":"NORMAL","required_action":"None","deadline_as_printed":None,"normalized_deadline":None,"affected_entities":[],"financial_amount":None,
      "full_text_ocr":text,"extraction_method":method,"extraction_confidence":"HIGH" if subject and printed else "MEDIUM","sensitive":False,"useful":True,"duplicate":False,"duplicate_reason":None,
      "processing_status":"Completed","forwarding_status":"Not Forwarded","approved_for_publication":False,"category_key":"other","category_source":"rule","category_confidence":"LOW","ai_suggestion_status":"Not Requested",
      "publication_status":"Unpublished","publication_reason":"Awaiting publication workflow","public_revision":0
    }
    ...
    print(f"Processed {rid} -> {actual_id} via {method}"); return True

def main():
    rows=db_get("telegram_intake?select=*&or=(status.eq.Stored,status.eq.Processing%20Failed)&order=received_at.asc&limit=10"); processed=failed=0
    for row in rows:
        try:
            if process(row): processed+=1
        except Exception as exc:
            failed+=1; md=row.get("metadata") or {}
            db_patch("telegram_intake",row["id"],{"status":"Processing Failed","metadata":{**md,"processing_error":str(exc)[:1200],"processing_error_at":datetime.now(timezone.utc).isoformat()}})
            print(f"Record {row['id']}: processing failed: {exc}")
    print(f"Document processor complete: processed={processed}, failed={failed}, candidates={len(rows)}"); return 1 if failed else 0
