...
def score_metadata_quality(metadata: dict[str, Any], text: str) -> dict[str, Any]:
    ...
    evidence = {name: bool(value and str(value).casefold() in evidence_text) for name, value in {"subject": subject, "authority": authority, "reference_number": reference, "issue_date": date}.items()}
...
def process_pdf(pdf_path: str, work_dir: str, *, min_embedded_chars: int = 80) -> dict[str, Any]:
    """Extract OCR, normalized metadata, and evidence-based government intelligence."""
    path = Path(pdf_path)
    if not path.exists(): raise FileNotFoundError(pdf_path)
    if path.suffix.lower() != ".pdf": raise ValueError("process_pdf currently accepts PDF files only")
    ...
    intelligence = analyze_document(text, result.get("subject"))
    if intelligence["authority"]["value"]: result["authority"] = intelligence["authority"]["value"]
    if intelligence["subject"]["value"]: result["subject"] = intelligence["subject"]["value"]
    result["government_intelligence"] = intelligence
    result["short_description"] = intelligence.get("short_description")
    result["actions"] = intelligence.get("actions", [])
    result["deadlines"] = intelligence.get("deadlines", [])
    result["document_type"] = intelligence.get("document_type")
