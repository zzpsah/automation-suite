...
The result contains extracted text, structured Sarkari metadata, extraction method, filename, and OCR service version. Consumers should tolerate missing metadata fields and must not treat OCR output as authoritative without appropriate validation.
...
4. `sarkari_normalizer.py` performs conservative cleanup and metadata extraction.
...
- Failed processing must remain observable in the consuming workflow.
- Text-only Telegram records must not be treated as PDF OCR jobs.
- OCR extraction must remain separate from publication/approval decisions.
