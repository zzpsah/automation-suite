function geminiSuggest_(file, extraction) {
  ...
  if (bytes.length > 50 * 1024 * 1024) {
    throw new Error('Gemini inline document limit exceeded');
  }

  const prompt = [
    'You are a careful school-document indexing assistant.',
    ...
    'The original file must not be treated as renamed or approved.',
    'Use null when a value is not visible on page 1.',
    'Allowed priority values: URGENT, HIGH, NORMAL, LOW, IGNORE.',
    'Allowed category examples include BSEB, Examination, Registration, Student,',
    'Admission, Payment / Fee, Scholarship, UDISE, School Administration, Teacher / Staff,',
    'Inspection, Meeting, Training, Government Order, District Office, Block Office,',
    'Notice / Circular, Academic, Data Submission, Portal / Technical Issue, Finance / Accounts,',
  ...
  const schema = {
    ...
    required: [
      ...
      'portal_description', 'display_filename', 'confidence', 'notes'
    ]
  };
...
function sanitizeSuggestedFilename_(value, originalName) {
  ...
  return name.toLowerCase().endsWith('.pdf') || name.toLowerCase().endsWith('.jpg') ||
...
function geminiSuggestionChanges_(suggestion) {
  return {
    ...
    ai_model: suggestion.model || suggestion.provider || 'unknown',
    ai_suggested_title: suggestion.title || null,
    ai_suggested_display_filename: suggestion.display_filename || null,
    ai_suggested_description: suggestion.portal_description || null,
    ai_suggested_json: suggestion,
...
function formatGeminiTelegramReport_(suggestion) {
  return [
    ...
    'Status: Not approved; original file unchanged.'
  ].join('\n');
}
