  ...
  async execute(request: NativeRequest): Promise<NativeResult> {
    ...
    try {
      switch (request.operation) {
        case "clipboard-read": {
          const text = await powershell("Get-Clipboard -Raw", process.env);
          return { ok: true, observation: { clipboardChanged: false }, error: undefined, } as NativeResult & { output?: string };
        }
        ...
        case "activate-window": {
          ...
          const process = boundedText(request.targetWindow?.process, 128);
          if (!title && !process) return { ok: false, observation: {}, error: { code: "MISSING_TARGET", message: "Window title or process is required.", recoverable: false } };
...
if(-not $proc){throw "Target window not found."}
[WWBUser32]::SetForegroundWindow($proc.MainWindowHandle) | Out-Null
$proc.Id
`, { ...process.env, WWB_TITLE: title, WWB_PROCESS: process });
          return { ok: true, observation: { foregroundWindow: { title: title || result, ...(process ? { process } : {}) } } };
        }
        case "keyboard": {
